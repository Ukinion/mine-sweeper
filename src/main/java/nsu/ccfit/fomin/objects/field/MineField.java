package nsu.ccfit.fomin.objects.field;

import nsu.ccfit.fomin.objects.unit.Cell;
import nsu.ccfit.fomin.exception.GameException;
import nsu.ccfit.fomin.exception.InvalidFieldSetupException;
...
public class MineField {
    ...
    private static final int MAX_FIELD_SIZE = 27;
    ...
    private boolean isInvalidOptions(int numRows, int numCols, int numMines) {
        return (numRows < MIN_FIELD_SIZE || numRows > MAX_FIELD_SIZE ||
                numCols < MIN_FIELD_SIZE || numCols > MAX_FIELD_SIZE ||
                numMines < MIN_MINES_ON_FIELD || numMines > numCols*numRows-2);
    }
    ...
    private boolean isInnerCell(int x, int y) {
        return ((x > 0 && x < _fieldCol-1) && (y > 0 && y < _fieldRow-1));
    }

    private boolean isBoundaryCell(int x, int y) {
        return ((x == 0 || x == _fieldCol-1) && (y != 0 && y != _fieldRow-1)) ||
                ((y == 0 || y == _fieldRow-1) && (x != 0 && x != _fieldCol-1));
    }
